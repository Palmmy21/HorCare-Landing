...
const APP_URL = 'https://hor-care.vercel.app/';
...
const TEXT_TRIGGERS = {
  ...
  'สมัคร HorCare รายเดือน': (e) => replyPackageQR(e, 'large', 'monthly'),
  ...
  'สมัคร HorCare รายปี': (e) => replyPackageQR(e, 'large', 'yearly'),

  // Renew / Change current package
  ...
  // Promotion
  'รับสิทธิ์โปร': replyPromotionQR,
  'สนใจโปร': replyPromotionQR,
};
  ...
  if (snapshot.empty) {
    return lineClient.replyMessage({
      ...
      messages: [
        {
          type: 'text',
          text: '📌 กรุณาส่ง Owner ID ของคุณมาในแชทนี้เพื่อผูกบัญชีครับ\n\n'
              + '🔍 หากมีบัญชีแล้ว (วิธีหา Owner ID):\n'
              + '1. เปิดแอป HorCare (' + APP_URL + ')\n'
              + '2. ไปที่ "ตั้งค่า" ⚙️ → "บัญชีผู้ใช้"\n'
              + '3. กดคัดลอก Owner ID แล้วส่งมาที่นี่ได้เลยครับ\n\n'
              + '💡 หากยังไม่มีบัญชี HorCare:\n'
              + '1. สมัครใช้งานฟรีที่: ' + APP_URL + '\n'
              + '2. เข้าไปที่ "ตั้งค่า" ⚙️ → "บัญชีผู้ใช้"\n'
              + '3. คัดลอก Owner ID แล้วส่งมาในแชทนี้ได้เลยครับ!\n\n'
    ...
    return lineClient.replyMessage({
      ...
      messages: [{
        ...
        text: '📌 กรุณาส่ง Owner ID ของคุณมาก่อนเพื่อผูกบัญชีครับ\n\n'
            + '🔍 หากมีบัญชีแล้ว: เปิดแอป HorCare → ตั้งค่า ⚙️ → บัญชีผู้ใช้ → คัดลอก Owner ID\n\n'
            + '💡 หากยังไม่มีบัญชี HorCare:\n'
            + '1. สมัครใช้งานฟรีที่: ' + APP_URL + '\n'
            + '2. คัดลอก Owner ID จากหน้าตั้งค่ามาส่งในแชทนี้ได้เลยครับ!',
      }],
    });
  }
...
// ── Generate QR for Promotion (100 THB off first month) ──────────────────────
async function replyPromotionQR(event) {
  const userId = event.source.userId;
  const snapshot = await db.collection('users')
    .where('line_support_user_id', '==', userId).limit(1).get();
  ...
  if (snapshot.empty) {
    return lineClient.replyMessage({
      ...
      messages: [{
        type: 'text',
        text: '📌 กรุณาส่ง Owner ID ของคุณมาก่อนเพื่อรับสิทธิ์โปรโมชันครับ\n\n'
            + '🔍 หากมีบัญชีแล้ว: เปิดแอป HorCare → ตั้งค่า ⚙️ → บัญชีผู้ใช้ → คัดลอก Owner ID\n\n'
            + '💡 หากยังไม่มีบัญชี HorCare:\n'
            + '1. สมัครใช้งานฟรีที่: ' + APP_URL + '\n'
            + '2. คัดลอก Owner ID จากหน้าตั้งค่ามาส่งในแชทนี้ได้เลยครับ!',
  ...
  const qrUrl = `https://promptpay.io/${PROMPTPAY_NUMBER}/${promoPrice}.png`;
  ...
  return lineClient.replyMessage({
    ...
    messages: [
      {
        type: 'text',
        text: `🎉 รับสิทธิ์โปรโมชันส่วนลด 100 บาท!\n\n`
            + `สมัครแพ็กเกจ HorCare รายเดือน\n`
            + `ยอดชำระ: ฿${promoPrice.toLocaleString()} (สำหรับเดือนแรก)\n\n`
            + `สแกน QR Code ด้านล่างเพื่อชำระเงิน\nหรือโอนเงินเข้าบัญชี:\n`
            + `พร้อมเพย์: ${PROMPTPAY_NUMBER}\n`
            + `ชื่อบัญชี: ${PROMPTPAY_NAME}\n\n`
            + `✅ เมื่อโอนเสร็จ ส่งสลิปมาในแชทนี้ได้เลยครับ`,
    ...
    messages: [
      {
        ...
        text: '🏠 ยินดีต้อนรับสู่ HorCare Support! 👋\n\n'
            ...
            + '📌 หากมีบัญชีระบบ HorCare แล้ว:\n'
            + '1. เปิดแอป HorCare (' + APP_URL + ')\n'
            + '2. ไปที่ "ตั้งค่า" ⚙️ → "บัญชีผู้ใช้"\n'
            + '3. กดคัดลอก Owner ID แล้วส่งมาที่นี่ได้เลยครับ\n\n'
            + '💡 หากยังไม่มีบัญชีระบบ HorCare:\n'
            + '1. สมัครใช้งานฟรีได้ที่: ' + APP_URL + '\n'
            + '2. เมื่อสมัครแล้ว ไปที่ "ตั้งค่า" ⚙️ → "บัญชีผู้ใช้"\n'
            + '3. คัดลอก Owner ID แล้วส่งมาในแชทนี้เพื่อเริ่มใช้งานครับ!',
      },
    ],
  });
}
...
export async function handleMessage(event) {
  ...
  let handler = TEXT_TRIGGERS[text];
  if (!handler) {
    // Fallback: search if message text includes any trigger key
    const matchedKey = Object.keys(TEXT_TRIGGERS).find(k => text.includes(k) || k.includes(text));
    if (matchedKey) handler = TEXT_TRIGGERS[matchedKey];
  }
